...
func newWatcher(ctx context.Context, key, name string, client *clientv3.Client) (*watcher, error) {
	...
	w.watchChan = w.watcher.Watch(w.ctx, key, getWatchOpOption()...)
...
func (w *watcher) getInstance() ([]*registry.Instance, error) {
	...
	for _, kv := range resp.Kvs {
		...
		if w.srvName != "" && si.Name != w.srvName {
			continue
		}
...
func (w *watcher) reWatch() error {
	...
	w.watchChan = w.watcher.Watch(w.ctx, w.key, getWatchOpOption()...)
	return w.watcher.RequestProgress(w.ctx)
}

func getWatchOpOption() []clientv3.OpOption {
