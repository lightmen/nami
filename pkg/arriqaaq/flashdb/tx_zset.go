...
func (tx *Tx) ZRemMemByRank(key string, start, end int) (zs int) {
	if tx.db.hasExpired(key, ZSet) {
		tx.db.evict(key, ZSet)
		return
	}

	zsu := tx.db.zsetStore.ZRemMemByRank(key, uint64(start), uint64(end))

	zs = int(zsu)
	e := newRecord([]byte(key), nil, ZSetRecord, ZSetZRemByRank)
	tx.addRecord(e)
