...
func (z *zskiplist) deleteRangeByRank(start, end uint64, dict map[string]*zskiplistNode) uint64 {
	update := make([]*zskiplistNode, SKIPLIST_MAXLEVEL)
...
func (z *zset) findRange2(key string, start, stop int64, reverse bool, f func(float64, string, interface{})) {
	length := z.zsl.length
...
func (z *ZSet) ZRemMemByRank(key string, start, end uint64) uint64 {
	if !z.exist(key) {
		return 0
	}
	...
	n := z.records[key]
	zsl := n.zsl
	return zsl.deleteRangeByRank(start, end, n.dict)
}
...
func (z *ZSet) ZRevRangeWithFunc(key string, start, stop int, f func(float64, string, interface{})) {
	if !z.exist(key) {
		return
	}

	n := z.records[key]
	n.findRange2(key, int64(start), int64(stop), true, f)
	return
}
